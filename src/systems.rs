...
pub fn fly_mode_keyboard(
	...
	for (mut camera, mut camera_transform, mut projection) in q_camera.iter_mut() {
		...
		let direction = (strafe_vector(&rotation) * axis_h) + (forward_walk_vector(&rotation) * axis_v) + (Vec3::Y * axis_float);

		let velocity = direction.normalize_or_zero() * camera.speed * delta_seconds;

		let from			= camera_transform.translation;
		let to				= camera_transform.translation + velocity;

		let inertia			= (delta_seconds / 0.1).min(1.0);
